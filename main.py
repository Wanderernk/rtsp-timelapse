...
def create_timelapse(subfolder, force_framerate=False):
    """
    ...
    if force_framerate:
        ...
        timelapse_filepath = f"{timelapse_directory}/{subfolder}/forced_fps/{timelapse_filename}"
        subprocess.run(
            [
                "ffmpeg",
                "-r",
                ...
                "glob",
                "-i",
                f"{images_directory}/{subfolder}/*.png",
        ...
        timelapse_filepath = f"{timelapse_directory}/{subfolder}/normal_fps/{timelapse_filename}"
        subprocess.run(
            [
                "ffmpeg",
                "-pattern_type",
...
def main():
    ...
    for camera_ip_address in rtsp_ip_address:
        rtsp_path = f"rtsp://{rtsp_username}:{rtsp_password}@{camera_ip_address}/stream1"
        ...
        image_files = glob.glob1(camera_dir, "*.png")
